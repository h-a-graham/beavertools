...
#' generate a panel plot of multiple forage density plots generated with `plot_forage_density`
#'
#'
#' @export
panel_plot_forage_density <- function(forage_plot_list, scalebar=TRUE, scalebar_loc = 'tl',
                                     north_arrow = TRUE, north_arrow_loc = 'br', north_arrow_size = 0.5){
  n <- length(forage_plot_list)

  if (isTRUE(scalebar)){
    forage_plot_list[[n]] <- forage_plot_list[[n]] +
  ...
  nCol <- ceiling(sqrt(n))
  do.call(gridExtra::grid.arrange, c(forage_plot_list, ncol=nCol))
}
