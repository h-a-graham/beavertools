...
plot_territories <- function(
  ...
  if (!is.null(catchment)) {
    ...
    p <- p +
      ggspatial::annotation_spatial(catch_mask, fill = mask_fill, alpha = 0.5)
  }

  if (isTRUE(rivers)) {
    if (is.null(catchment)) {
      message(
        '"rivers" argument ignored. A valid "catchment" area is required.'
      )
    } else {
  ...
  if (fill_name == 'feedsigns') {
    ...
    if (length(weight_levs) == 3) {
      p <- p +
        ggplot2::geom_sf(
          ...
          mapping = ggplot2::aes(
            colour = FeedCat,
            size = !!dplyr::sym(fill_name)
          ),
          alpha = 0.7,
          inherit.aes = FALSE
        ) +
        ggplot2::scale_colour_manual(
          values = c(fill_col[1], fill_col[2], fill_col[3]),
          breaks = (c('Low', 'Med', 'High')),
          labels = (c('Low', 'Med', 'High')),
          name = 'Impact level'
        ) +
        ggplot2::scale_size(
          range = c(weight_levs[3], weight_levs[1]),
      ...
      p <- p +
        ggplot2::geom_sf(
          terr_poly,
          mapping = ggplot2::aes(colour = FeedCat),
          alpha = 0.7,
          size = weight_levs
        ) +
        ggplot2::scale_colour_manual(
    ...
    p <- p +
      ggplot2::geom_sf(
        terr_poly,
        mapping = ggplot2::aes(colour = othersigns),
        alpha = 0.5,
        size = dplyr::pull(terr_poly, p_size)[1]
      ) +
      ggplot2::scale_colour_manual(
        values = c(fill_col[1], fill_col[2]),
        name = 'Sign type',
        labels = c('Dam', 'Dwelling'),
        breaks = c('Dam', 'Dwelling'),
        drop = FALSE
      )
  } else {
    # Now we tackle polygon requests
    ...
    p <- p +
      ggplot2::geom_sf(
        terr_poly,
        mapping = ggplot2::aes(fill = !!dplyr::sym(fill_name)),
        alpha = 0.7,
        lwd = 0.3
      )
  ...
  if (isTRUE(label)) {
    p <- p +
      ggplot2::geom_sf_label(
        dplyr::filter(terr_poly, terr_status != 'Activity'),
