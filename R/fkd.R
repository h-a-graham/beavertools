...
#'
forage_density <- function(
  forage_points,
  impact_cat,
  grid_size = 20,
  kern_bw = 250,
  kd_extent,
  kd_weights = c(1, 1e+03, 1e+06),
  low_thresh = 1e-12,
  standardise = FALSE
) {
  ...
  if (missing(impact_cat)) {
    message(
      'WARNING: No value supplied for "impact_cat" argument. Kernel density estimate will not be weighted.'
    )
    weighted_kde <- FALSE
  } else {
    forage_points <- forage_points %>%
      dplyr::mutate(
        weights = ifelse(
          !!dplyr::sym(impact_cat) == 'Low',
          kd_weights[1],
          ifelse(
            !!dplyr::sym(impact_cat) == 'Med',
            kd_weights[2],
            ifelse(!!dplyr::sym(impact_cat) == 'High', kd_weights[3], NA)
          )
        )
      )
    weighted_kde <- TRUE
  ...
  } else if (inherits(kd_extent, "bbox")) {
    kd_extent <- define_extent_bbox(kd_extent, grid_size)
  } else if (
    inherits(kd_extent, "sf") ||
      inherits(kd_extent, "SpatialPointsDataFrame") ||
      inherits(kd_extent, "raster")
  ) {
    kd_extent <- define_extent_sp(kd_extent, grid_size)
  } else if (inherits(kd_extent, "character")) {
  ...
  if (isTRUE(weighted_kde)) {
    KernDen <- spatialEco::sf.kde(
      sf::st_as_sf(forage_points),
      y = forage_points$weights,
      bw = kern_bw,
      ref = kd_extent,
      res = grid_size,
  ...
  if (sf::st_crs(KernDen) != sf::st_crs(forage_points)) {
    KernDen <- terra::project(
      KernDen,
      sf::st_crs(forage_points)$wkt
    )
  }
